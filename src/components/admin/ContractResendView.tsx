...
const ContractResendView = () => {
  ...
  const fetchUsers = async () => {
    setLoading(true);
    try {
      // Fetch approved UGC creators with contract info
      const { data, error } = await supabase
        .from('users')
        .select('id, full_name, email, application_status, approved_at, contract_sent_at, contract_signed_at, signwell_document_id')
        .eq('role', 'ugc_creator')
      ...
      // Cast to our interface since TypeScript types may be outdated
      setUsers((data || []) as unknown as UserWithContract[]);
    } catch (error) {
  ...
  const sendContract = async (user: UserWithContract) => {
  ...
  const getContractStatus = (user: UserWithContract) => {
    // Signed = best status
  ...
  const usersSigned = usersWithContract.filter(u => u.contract_signed_at);
  const usersPendingSignature = usersWithContract.filter(u => !u.contract_signed_at);
                    ...
                    <Card key={user.id} className="p-4 bg-red-500/5 border-red-500/20">
                      <div className="flex flex-col gap-3">
                        ...
                        <div className="text-sm text-muted-foreground space-y-1">
                          ...
                          {user.contract_sent_at && (
                            <div className="text-xs">Sent: {formatDate(user.contract_sent_at)}</div>
                          )}
                        </div>
                        <Button
                  ...
                  return (
                    <Card key={user.id} className="p-4 bg-green-500/5 border-green-500/20">
                      <div className="flex flex-col gap-3">
                        ...
                        <div className="text-sm text-muted-foreground space-y-1">
                          <div className="truncate">{user.email}</div>
                          <div className="text-xs">Signed: {formatDate(user.contract_signed_at)}</div>
                        </div>
                        <Button
                          onClick={() => sendContract(user)}
                          disabled={isSending}
...
export default ContractResendView;
